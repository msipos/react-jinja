function parsing2(input) {
  ...
  var stateFor = function() {
    ...
    var nd = nextNode();
    if (nd === null || nd.type != '%') {
      throw new ParsingException('Invalid start of %for');
    }
    if (nd.percType != 'for') {
      throw new ParsingException('Invalid start of %for');
    }
    ...
    for (;;) {
      ...
      if (nd === null) {
        throw new ParsingException('Unterminated %for');
      }
      ...
      if (nd.type === '%') {
        if (nd.percType === 'endfor') {
          consumeNode();
          return node;
        } else {
          destArray.push(stateStartPerc());
        }
      }
    }
  };
  ...
  var stateStartPerc = function() {
    ...
    if (nd.percType === 'if') {
      return stateIf();
    } else if (nd.percType === "for") {
      return stateFor();
    } else {
      throw new ParsingException('Invalid % statement: ' + nd.percType);
    }
  }
