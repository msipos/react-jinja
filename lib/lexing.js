function lexing(input) {
  // Preprocess: trim whitespace on both ends.
  input = input.trim();
  ...
  var tokens = state.output;

  /** Preprocess tokens. **/

  // TODO: Ignore tokens.

  // Preprocess lexed tokens to remove whitespace agglomerations at the top
  var i, token;
  for (i = 0; i < tokens.length; i++) {
    token = tokens[i];
    if (token.type !== "aggl" || token.text.trim() !== "") {
      break;
    }
  }
  tokens = tokens.slice(i);

  // Remove whitespace agglomerations at the bottom
